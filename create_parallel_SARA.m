...
function create_parallel_SARA(parallel_fun, job_list1, job_list2, job_list3, parallel_fun_dir, job_name)
%% basic input checks
...
project_dir = '/home/ehvbeest/SVM_STIMDEC' % must be the ABSOLUT path
...
for job_ind = 1:length(job_list1)
    for job_ind2 = 1:length(job_list2)
        for job_ind3 = 1:length(job_list3)
            curr_job = [job_list1(job_ind), job_list2(job_ind2), job_ind3];
            
            %% create batchfile for current job
            % create/overwrite file
            filename = sprintf('run_job%02i_%02i_%02i_%s.sh', curr_job(1), curr_job(2), curr_job(3), job_name);
            fullfilename = [batch_dir '/' filename]
            
            display(['Creating Batch file for Job ' num2str(curr_job(1)) '_' num2str(curr_job(2)) '_' num2str(curr_job(3)) ': ' fullfilename])
            ...
            line = sprintf('%s %s %i %i %i %s %s', execute_matlab_process_sh, parallel_fun, curr_job(1), curr_job(2), curr_job(3), log_file_dir, parallel_fun_dir);
            ...
            fprintf(fid_commit_all, '# Job %i_%i_%i\n', curr_job(1), curr_job(2), curr_job(3));
