...
n = -5:1:15; %Range of what parameter c could be (c = 2^n(i))
gopt = -15:1:3 %% Parameter g (g = 2^gopt(i))
...
%     randomtidx = shamtrialvec(find(shamtrialvec<=length(nrtrials)*takenrt));
    ...
    try
        ...
        p=4;
        XDat = zeros(2,takenrt,(xpix*ypix)/(p^2),'single');
        YDat = zeros(2,nrtask,takenrt,'single');
        for mid = 1:length(m)
            ...
            for trialidx = 1:takenrt
                ...
                XDat(mid-2,trialidx,:) = tmp2;
            end
            names = strsplit(m{mid},'_');
%             YDat(1,(mid-1)*takenrt+1:mid*takenrt) = repmat(find(strcmp(BGOptNames,names{1})),[takenrt,1]);
            YDat(mid-2,1,:) = repmat(find(strcmp(SideOpt,names(2))),[takenrt,1]);
        end
        throwpix = find(sum(sum(isnan(XDat),1),2)>0);
        XDat(:,:,throwpix) = []; %Remove any pixels that have missing data or have too low variance
        YDat = squeeze(YDat);
        %Z-score per trial over pixels
        stop = 0;
        count = 1;
        while count<10 && ~stop
            ZSc = (XDat - reshape(repmat(nanmean(reshape(XDat,[size(XDat,1)*size(XDat,2),size(XDat,3)]),2),[1,size(XDat,3)]),[size(XDat,1),size(XDat,2),size(XDat,3)]))./reshape(repmat(nanstd(reshape(XDat,[size(XDat,1)*size(XDat,2),size(XDat,3)]),[],2),[1,size(XDat,3)]),[size(XDat,1),size(XDat,2),size(XDat,3)]);
            if any(abs(ZSc(:))>2)
                ZSc(ZSc>2)= 2;
                ZSc(ZSc<-2)=-2;
            else
        ...
        chunksz = floor(takenrt./nfolds); %Even chunks of data as nested-cross per side

    catch ME
        disp(ME)
        keyboard
    end
    actvspred = nan(nrtask,takenrt*2,2);
    shamtrialvec1 = randperm(takenrt);
    shamtrialvec2 = randperm(takenrt);
    randomtidx = cat(1,shamtrialvec1,shamtrialvec2);
    ...
    for nc = 1:nfolds
        ...
        for i = 1:size(randomtidx,1)
            trainingcidx(i,:) = randomtidx(i,~ismember(randomtidx(i,:),testcidx(i,:)));
            training = cat(1,training,squeeze(XDat(i,trainingcidx(i,:),:)));
            trainlab = cat(2,trainlab,YDat(i,trainingcidx(i,:)));
        end
        ...
        testlab = [];
        for i = 1:size(randomtidx,1)
            test = cat(1,test,squeeze(XDat(i,testcidx(i,:),:)));
            testlab = cat(2,testlab,YDat(i,testcidx(i,:)));
        end
       
        % Normalize between 0 and 1
        test = (test-repmat(mindat,[size(test,1),1]))./repmat((maxdat-mindat),[size(test,1),1]);
       
        model = fitcsvm(training,trainlab);
        beta = model.Beta %Feature weights
        
        figure
        pixelmap = nan(xpix/p,ypix/p);
        keeppix = 1:prod(size(pixelmap));
        keeppix(throwpix) = [];
        pixelmap(keeppix)= beta;
        h = imagesc(pixelmap',[0.5 1]);
        set(h,'AlphaData',~isnan(pixelmap'))
        colorbar
      
        
        label = predict(model,test);
        acc = sum(testlab == label')./length(testlab);
        
        sv = model.SupportVectors;
        figure;
        try
            % Optimize parameter c on sub-set
            accuracy = nan(nrtask,length(n),length(gopt));
            mse = accuracy;
            ...
            for t = 1:nrtask
                for i = 1:numel(n) %
                    for j = 1:numel(gopt)
                        c = 2.^n(i);
                        g = 2.^gopt(j);
                        model = svmtrain(double(trainlab(nestedtrainidx))',double(training(nestedtrainidx,:)),['-t 1 -q -c ' num2str(c) ' -g ' num2str(g)]);
                        [lbl, acc, dec] = svmpredict(double(trainlab(t,nestedtestidx))',double(training(nestedtestidx,:)),model,['-q']);
                        accuracy(t,i,j) = acc(1);
                        mse(t,i,j) = acc(2);
                    end
                end
...
%                 
                if 1
                    ...
                    subplot(2,nrtask,(t-1)*2+1); h = imagesc(pixelmap',[0.5 1]);
                    set(h,'AlphaData',~isnan(pixelmap'))
                    ...
                    subplot(2,nrtask,(t-1)*2+2); h = imagesc(pixelmap');
                    set(h,'AlphaData',~isnan(pixelmap'))
                    axis square
                ...
                %%  Actual prediction
%                 model = svmtrain(double(trainlab(t,:))',double(training(:,ftRank(ridx))),['-t 1 -q -c ' num2str(2^n(acc_maxid(find(ismember(acc_maxid,MSE_minid),1))))]);
%                 [lbl, acc, dec] = svmpredict(double(testlab(t,:))',double(test(:,ftRank(ridx))),model,['-q']);
                model = svmtrain(double(trainlab(t,:))',double(training),['-t 1 -q -c ' num2str(2^n(acc_maxid(find(ismember(acc_maxid,MSE_minid),1))))]);
